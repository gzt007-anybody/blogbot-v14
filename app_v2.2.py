...
def render_cartoon_section(state_prefix: str, article_title: str, article_body: str, mode_label: str, default_tone: str = "일상 공감형"):
    ...
    with st.expander("🗞️ 갈맬 4컷 만화 만들기", expanded=False):
        st.caption("1) 먼저 스토리를 만든 뒤 2) 내용을 확인하고 3) 승인된 스토리로 만화 이미지를 생성합니다.")
        c1, c2 = st.columns(2)
        with c1:
            tone = st.selectbox("만화 톤", ["일상 공감형", "생활 정보형", "시사 풍자형"], index=["일상 공감형", "생활 정보형", "시사 풍자형"].index(default_tone) if default_tone in ["일상 공감형", "생활 정보형", "시사 풍자형"] else 0, key=f"{state_prefix}_tone")
        with c2:
            style_name = st.selectbox("이미지 스타일", list(COMIC_STYLE_PRESETS.keys()), index=0, key=f"{state_prefix}_style")

        if st.button("📝 갈맬 4컷 스토리 만들기", key=f"{state_prefix}_story_btn", use_container_width=True):
            if not api_key.strip():
                st.error("OPENAI_API_KEY를 확인해주세요.")
            else:
                try:
                    client = OpenAI(api_key=api_key.strip())
                    with st.spinner("갈맬 4컷 스토리를 만드는 중입니다..."):
                        st.session_state[story_key] = _make_galmael_story(client, model, article_title, article_body, mode_label, tone)
                    st.session_state.pop(image_key, None)
                    st.success("4컷 스토리 생성 완료")
                except Exception as e:
                    st.error("스토리 생성 오류: " + str(e))

        story = st.session_state.get(story_key)
        if isinstance(story, dict):
            st.markdown("### 갈맬 4컷 스토리")
            st.markdown(f"**제목:** {story.get('comic_title','갈맬 4컷 만화')}")
            if story.get("overall_theme"):
                st.caption("주제: " + str(story.get("overall_theme")))
            for panel in story.get("panels", []):
                if not isinstance(panel, dict):
                    continue
                pn = panel.get("panel", "")
                cap = str(panel.get("caption", "")).strip()
                with st.container(border=True):
                    st.markdown(f"**{pn}컷 — {cap}**")
                    for label, key in [("장면", "scene"), ("갈맬 대사", "dialogue_main"), ("보조 대사", "dialogue_sub"), ("내레이션", "narration")]:
                        val = str(panel.get(key, "")).strip()
                        if val:
                            st.markdown(f"- **{label}**: {val}")
            if story.get("closing_line"):
                st.info("갈맬의 마무리 한마디: " + str(story.get("closing_line")))

            st.download_button(
                "⬇️ 4컷 스토리 TXT 저장",
                _story_to_text(story),
                file_name=f"{state_prefix}_galmael_story.txt",
                mime="text/plain",
                key=f"{state_prefix}_story_txt"
            )

            if st.button("🎨 승인된 스토리로 4컷 만화 이미지 만들기", key=f"{state_prefix}_image_btn", use_container_width=True):
                if not api_key.strip():
                    st.error("OPENAI_API_KEY를 확인해주세요.")
                else:
                    try:
                        client = OpenAI(api_key=api_key.strip())
                        image_prompt = _build_galmael_image_prompt(story, style_name)
                        with st.spinner("갈맬 4컷 만화 이미지를 생성하는 중입니다..."):
                            st.session_state[image_key] = _generate_image_bytes(client, image_prompt)
                        st.success("4컷 만화 이미지 생성 완료")
                    except Exception as e:
...
3. 슬롯마다 첫 문장, 생활 예시, 소제목, 표 위치, 실천 팁이 반복되지 않게 한다.
